{{
  config(
    ...
    unique_key=var('snowplow__subscription_id'),
...
with first_subscription_event AS (
  select
    ...
    {% if target.type in ['postgres','spark'] %}
          , row_number() over (partition by ev.{{var('snowplow__subscription_id')}} order by ev.cv_tstamp) as session_dedupe_index
    {% endif %}

  from {{ ref('subscription_events_this_run' )}} as ev
  {% if target.type not in ['postgres','spark'] %}
    qualify row_number() over (partition by ev.{{var('snowplow__subscription_id')}} order by ev.cv_tstamp) = 1
  {% endif %}
),
...
select
  f.{{var('snowplow__subscription_id')}},
  f.user_identifier,
  f.user_id,
  f.stitched_user_id,
...
on 
  f.{{var('snowplow__subscription_id')}} = agg.{{var('snowplow__subscription_id')}}
{% if target.type in ['postgres','spark'] %}
